...
class Spider():
    ...
    def get_base_info(self,*args):
        ...
        for index,flight_info in enumerate(query_result):
            # flight_id = 'flight_No_%02d'%(index+1)
            sub_html = etree.HTML(flight_info)
            imgurls = sub_html.xpath('//div[@class="li_com"]/span/img/@src')
            ...
            # 航空公司名称和航班号
            time_and_place_info= self.__strip_blinks(sub_html.xpath('//div[@class="li_com"]/span[@class="w150"]/text()'))
            ...
            # flight_detailed_info_url = self.base_url + sub_html.xpath('//li[@style="position: relative;"]/a[@class="searchlist_innerli"]/@href')
            # 这一项暂时没用
            ...
            ontime_rate = self.img_mission_queue.put((ontime_rate_imgurl,ontime_rate_imgpath))
...
if __name__ == '__main__':
    ...
    myPrint(Spider().get_base_info('CA911','20190328'))
