...
class Spider():
    ...
    def _get_img(self,arg_tuple):
        # 获取图片，顺便识别图片内容
        # 参数最多只能有四个，至少两个，使用时请严格按照以下顺序：imgurl, img_path, return_type
        # return type 是一个数字，取值范围为0,1,2，1表示返回识别结果，0表示返回图片二进制内容，2表示返回元组（识别结果，二进制内容）
        # 正常情况下不返回结果，只更新self.img_result_dict字典
        imgurl = arg_tuple[0]
        img_path = arg_tuple[1]
        return_type = arg_tuple[2]
        try:
    ...
    def get_base_info(self,*args):
        ...
        for index,flight_info in enumerate(query_result):
            ...
            self.img_mission_queue.put((corp_imgurl,corp_imgpath,0))
            ...
            if dep_time_act_imgurl:
                self.img_mission_queue.put((dep_time_act_imgurl,dep_time_act_imgpath,1))
            ...
            if arri_time_act_imgurl:
                self.img_mission_queue.put((arri_time_act_imgurl,arri_time_act_imgpath,1))
        ...
        if reverse_info.group(1)=="2,1":
            for flight_info in flight_infos:
                flight_info['dep_time_act'],flight_info['arri_time_act'] = flight_info['arri_time_act'],flight_info['dep_time_act']
        return flight_infos
    def get_detailed_info(self,*args):
        ...
        for item in airp_weathers_iconurls:
            self.img_mission_queue.put((item,"./spider/img_file/%s.png"%item[item.rindex("/")+1:],0))
            airp_weathers_icons.append("./spider/img_file/%s.png"%item[item.rindex("/")+1:])
