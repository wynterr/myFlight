...
class Spider():
    ...
    def get_base_info(self,*args):
        ...
        for index,flight_info in enumerate(query_result):
            ...
            # 准点率
            ontime_rate_imgpath = "./img_file/%s_%s-%s_%s_onTimeRate.png"%(flight_code,dep_airp,arri_airp,query_date)
            if ontime_rate_imgurl:
                ...
                ontime_rate = '--'
            # 实际起飞时间
            dep_time_act_imgpath = "./img_file/dep_time_act_%s_%s-%s_%s.png"%(flight_code,dep_airp,arri_airp,query_date)
            if dep_time_act_imgurl:
                ...
                dep_time_act = '--'
            # 实际到达时间
            arri_time_act_imgpath = "./img_file/arri_time_act_%s_%s-%s_%s.png"%(flight_code,dep_airp,arri_airp,query_date)
    ...
    def get_detailed_info(self,*args):
        ...
        ave_ontime_rate_imgpath ="./img_file/%s_%s-%s_ave_ontime_rate.png"%(corp_name_and_flight_code,dep_city,arri_city)
        ...
        for item in airp_weathers_iconurls:
            self.img_mission_queue.put((item,"./img_file/%s.png"%item[item.rindex("/")+1:],False))
            ...
            for sub_i in range(airp_data_rows):
                ...
                # 共三列数据
                    ...
                    if not item_data_img_url:
                        ...
                        item_data_img_path = './img_file/%s_%s-%s_%s_%d%d%d.png'%(corp_name_and_flight_code,dep_city,arri_city,query_date,i+1,sub_i+1,j+1)
