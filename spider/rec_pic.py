...
# @Last Modified time: 2019-04-21 20:28:26
...
class  Rec_pic(object):
    def __init__(self):
        ...
        if not self.logger.handlers:
            ...
            file_formatter = logging.Formatter('[%(levelname)-8s] %(asctime)s %(filename)s[line:%(lineno)d] %(message)s')
            console_formatter = logging.Formatter('[%(levelname)-8s]%(filename)s[line:%(lineno)d] %(message)s')
    ...
    def combine(self,img_path_list,img_type):
        # 拼接指定数目的图片
        # 返回拼接图片的路径
        ...
        for index,img in enumerate(img_open_list):
            ...
            target.paste(img,pos_tuple)
                # print("位置：",pos_tuple)
        img_name = str(datetime.datetime.now()).replace(' ',"_").replace(":","-") + '.jpg'
    ...
    def check(self,combined_img_path,img_total,result,img_type):
        ...
        row_img_num = [img_col for i in range(img_row)]
        ...
        if img_type == 1:
            ...
            for index,item in enumerate(result):
                ...
                if len(item) != 4*row_img_num[index]:
                    self.logger.info("check函数判断识别结果有误！图片类型：%d，图片路径：%s，错误原因：漏识别数字，错误行：%s，识别结果行原文：%s"%(img_type,combined_img_path,item,result))
                    return False
            ...
            for index,rec_row in enumerate(result):
                ...
                for item in rec_row:
                    ...
                    item = item.replace(".","")
                    if not len(item)%4 == 0:
                        self.logger.info("图片类型：%d，图片路径：%s，错误原因：分片长度不是4的倍数，错误行：%s，识别结果行原文：%s"%(img_type,combined_img_path,list(rec_row),result))
                        return False
                    else:
                ...
                if len(revised_rec_row) != row_img_num[index]:
                    self.logger.info("图片类型：%d，图片路径：%s，错误原因：行四位数字组合不满足应有数目，错误行：%s，识别结果行原文：%s"%(img_type,combined_img_path,list(rec_row),result))
                    return False
            ...
            if not len(result) == img_total:
                self.logger.info("check函数判断识别结果有误！图片类型：%d，图片路径：%s，错误原因：识别结果行数不匹配，识别结果行原文：%s"%(img_type,combined_img_path,result))
                return False
            for rec_row in result:
                if len(rec_row.values())!=1:
                    self.logger.info("识别警告！图片类型：%d，图片路径：%s，警告内容：同一行有多个识别结果，已合并处理！识别结果行原文：%s"%(img_type,combined_img_path,result))
    ...
    def in_rec(self,img_path_list,img_type):
        ...
        if not result.get("error_code"):
            self.logger.debug("本次提交识别用时 %.3f s"%(time.time()-a))
            # self.logger.debug("识别原始结果：")
            # myPrint(result)
        else:
            self.logger.error("5 次提交均返回错误码，已放弃提交！")
    ...
    def test(self,img_type=None,img_total=None,test_times=1):
        ...
        for item in os.listdir(ORI_IMG_PATH):
            if not "商标" in item and item[-4:]==".png":
        ...
        while self.new_img_will_come or not mission_queue.empty():
            if mission_queue.empty():
                ...
                if len(type1_img_list) > 20:
                    self.logger.info("推送任务！任务图片类型：1")
                    t = threading.Thread(target=self.rec,args=(type1_img_list,1,result_dic));t.start()
                    self.rec_img_thread_list.append(t)
                    type1_img_list = []
                if len(type2_img_list) > 20:
                    self.logger.info("推送任务！任务图片类型：2")
                    t = threading.Thread(target=self.rec,args=(type2_img_list,2,result_dic));t.start()
                    self.rec_img_thread_list.append(t)
                    type2_img_list = []
                if len(type3_img_list) > 0:
                    self.logger.info("推送任务！任务图片类型：3")
                    t = threading.Thread(target=self.rec,args=(type3_img_list,3,result_dic));t.start()
                    self.rec_img_thread_list.append(t)
                    type3_img_list = []   
        if type1_img_list:
            self.logger.info("推送残余任务！任务图片类型：1，")
            t = threading.Thread(target=self.rec,args=(type1_img_list,1,result_dic));t.start()
            self.rec_img_thread_list.append(t)    
        if type2_img_list:
            self.logger.info("推送残余任务！任务图片类型：2，")
            t = threading.Thread(target=self.rec,args=(type2_img_list,2,result_dic));t.start()
            self.rec_img_thread_list.append(t)    
        if type3_img_list:
            self.logger.info("推送残余任务！任务图片类型：3，")
            t = threading.Thread(target=self.rec,args=(type3_img_list,3,result_dic));t.start()
            self.rec_img_thread_list.append(t)                
...
if __name__ == '__main__':
    ...
    # sample = [BASE_INFO_IMG_PATH+x for x in random.sample(os.listdir(BASE_INFO_IMG_PATH),6)]
