...
!((win) => {
    ...
    let showDateBox = (datedom) => {
        ...
        // 清除可能已有的日期框
        let olddom = document.getElementById("date-list").querySelectorAll('.' + dateboxCls);
        delDom(olddom);
        // 显示新的日期框
        datedom.style.left = thisleft;
        datedom.style.top = thistop;
        document.getElementById("date-list").append(datedom);
    ...
    let bindEvent_DaySelected = () => {
        ...
        dateboxDom.querySelectorAll('.date-item-day').forEach((item) => {
            item.onclick = (event) => {
                ...
                let thisobj = event.currentTarget;
                //
                let date = new Date(getAttr(thisobj, 'year'), getAttr(thisobj, 'month')
                    , getAttr(thisobj, 'day'), cfg.hour, cfg.minute, cfg.second);
    ...
    let bindEvent_OkBtn = () => {
        ...
        dateboxDom.querySelector('.date-btn-ok').onclick = (event) => {
            ...
            if (seledDay) {
                let d = new Date(getAttr(seledDay, 'year'), getAttr(seledDay, 'month'), getAttr(seledDay, 'day')
    ...
    mydate.close = () => {
        ...
        inputDOM = null;
        cfg = null;
        let datedoms = document.getElementById("date-list").querySelectorAll('.' + dateboxCls);
        delDom(datedoms);
    }
    // 点击日期控件以外区域,关闭控件.
    document.onclick= () => {
